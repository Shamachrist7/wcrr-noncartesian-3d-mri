...
params_algo = {"stepsize": 1/L, "lambda": 5e-4}
max_iter = 100
model = optim_builder(
    iteration="ADMM",
    prior=prior,
...
for i, volume in enumerate(volumes):
    ...
    wandb.log({"volume_idx": i, "psnr_zf": psnr(zf_recon, reference), "ssim_zf": ssim(zf_recon, reference), "psnr_grappa": psnr(grappa_recon, reference), "ssim_grappa": ssim(grappa_recon, reference), "psnr_wv": psnr(wv_recon, reference), "ssim_wv": ssim(wv_recon, reference)})
    if i < 5:
        ...
        torch.save(wv_recon, f"savings/volume_{i}_wv.pt")
