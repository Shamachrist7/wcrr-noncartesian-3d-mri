    ...
    def __init__(
        ...
        if pretrained is not None:
            if pretrained == "download":
                ...
                ckpt_drunet = torch.load(pretrained, map_location=device)
    ...
    def forward(self, x, sigma):
        ...
        if isinstance(sigma, torch.Tensor):
            ...
            if self.dim == 2:
                ...
                noise_level_map = (
                    torch.ones((x.size(0), 1, x.size(2), x.size(3), x.size(4)), device=x.device)
                    * sigma
                )
        ...
        if self.training or (
            D % 8 == 0 and H % 8 == 0 and W % 8 == 0 and D > 31 and H > 31 and W > 31
        ):
            x = self.forward_unet(x)                     # returns [B, C_out, D, H, W]
        elif D < 32 or H < 32 or W < 32:
            x = test_pad3d(self.forward_unet, x, modulo=16)
        else:
            x = test_onesplit3d(self.forward_unet, x, refield=64)  # overlap defaults to 32
...
# def update_keyvals(old_key, old_value, init_value=None, new_key_name='m_head.conv0.weight'):
...
# --------------------------------------------
# routines to handle the input dimensions that are not multiple of 8 inside the DRUNet
# --------------------------------------------
import torch
import torch.nn.functional as F
import math

def _pad_to_multiple_3d(x, modulo=16, mode="reflect"):
    _, _, D, H, W = x.shape
    def need(sz):
        r = sz % modulo
        return 0 if r == 0 else (modulo - r)
    padW = need(W); padH = need(H); padD = need(D)
    pad = (0, padW, 0, padH, 0, padD)  # (w0,w1,h0,h1,d0,d1)
    if any(pad):
        if mode == "reflect":
...
def _crop_from_pad_3d(x, pad):
    pw0, pw1, ph0, ph1, pd0, pd1 = pad
    if not any(pad):
        return x
    return x[..., 0:x.size(-3)-pd1, 0:x.size(-2)-ph1, 0:x.size(-1)-pw1]

def test_pad3d(f, x, modulo=16):
    x_pad, pad = _pad_to_multiple_3d(x, modulo=modulo, mode="reflect")
    y_pad = f(x_pad)
    y = _crop_from_pad_3d(y_pad, pad)
    return y

def test_onesplit3d(f, x, refield=64, overlap=None):
    """
    Tiled inference for [B,C_in,D,H,W] -> [B,C_out,D,H,W].
    Does NOT assume C_out == C_in. Allocates accumulators after first tile forward.
    """
    if overlap is None:
        overlap = refield // 2
    ...
    # choose tile size
    tileD = min(D, refield + overlap)
    tileH = min(H, refield + overlap)
    tileW = min(W, refield + overlap)

    strideD = max(refield, 1)
    strideH = max(refield, 1)
    strideW = max(refield, 1)

    # smooth blending window (broadcasts over channels)
    def win_1d(tile_n, ref):
        ov = max(tile_n - ref, 0)
        if ov == 0:
            return torch.ones(tile_n, device=device, dtype=dtype)
        t = torch.arange(tile_n, device=device, dtype=dtype)
        left  = 0.5 * (1 - torch.cos(math.pi * torch.clamp(t / ov, 0, 1)))
        right = 0.5 * (1 - torch.cos(math.pi * torch.clamp((tile_n - 1 - t) / ov, 0, 1)))
        return torch.minimum(left, right).clamp(min=1e-3)
    wD = win_1d(tileD, refield).view(1, 1, tileD, 1, 1)
    wH = win_1d(tileH, refield).view(1, 1, 1, tileH, 1)
    wW = win_1d(tileW, refield).view(1, 1, 1, 1, tileW)
    window = (wD * wH * wW).to(dtype)

    out = None  # will be allocated after first pred
    wgt = None

    for b in range(B):
        dz_list = range(0, max(D - refield, 0) + 1, strideD)
        hy_list = range(0, max(H - refield, 0) + 1, strideH)
        wx_list = range(0, max(W - refield, 0) + 1, strideW)
        for dz in dz_list:
            d0 = min(dz, D - tileD); d1 = d0 + tileD
            for hy in hy_list:
                h0 = min(hy, H - tileH); h1 = h0 + tileH
                for wx in wx_list:
                    w0 = min(wx, W - tileW); w1 = w0 + tileW

                    tile = x[b:b+1, :, d0:d1, h0:h1, w0:w1]
                    pred = test_pad3d(f, tile, modulo=16)  # [1, C_out, td, th, tw]

                    # allocate accumulators on first tile using C_out
                    if out is None:
                        C_out = pred.size(1)
                        out = torch.zeros(B, C_out, D, H, W, device=device, dtype=dtype)
                        wgt = torch.zeros(B, C_out, D, H, W, device=device, dtype=dtype)

                    out[b:b+1, :, d0:d1, h0:h1, w0:w1] += pred * window
                    wgt[b:b+1, :, d0:d1, h0:h1, w0:w1] += window

    out = out / wgt.clamp_min(1e-16)
    return out
