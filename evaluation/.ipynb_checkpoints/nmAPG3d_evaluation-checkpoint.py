...
def nmAPG(
    ...
    x_gt: torch.Tensor = None, # Ground Truth! If provided, the PSNR history will be computed
    max_iter: int = 200,
    L_init: float = 1.0,
    tol: float = 1e-4,
    rho: float = 0.9,
    ...
    for i in tqdm(range(max_iter)):
        ...
        # Save energy history
        energy_history.append(f(x, y, sigma).item())
    ...
    if use_amp:
        x = x.float()
    return x, L, i, res_history, energy_history, psnr_history
...
def reconstruct_nmAPG(
    ...
    def energy(val, y_in, sigma):
        with torch.no_grad():
            en = data_fidelity(val, y_in, physics) + lamda * regularizer.g(val, sigma) # We sum over the coils
        return en.reshape(-1)
    ...
    # example energies
    rec, L, steps, residuals, energies, psnrs = nmAPG(
        sigma,
        x0=x0,
        y=y,
        f=energy,
    ...
    stats = dict(L=L.detach(), steps=steps, residual=residuals, cost=energies, psnr=psnrs)
    if return_stats:
        return rec, stats
    return rec
